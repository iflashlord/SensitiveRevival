/**
 * EnhancedLevelManager: We copied the original CitrusEngine LevelManager to add the following addition:
 * Levels loaded via SWF are stored in memory and are re-used when the same level is restarted:
 * This prevents re-loading of the SWF, which is not supported in IOS AOT mode (built IOS package).
 * 
 * (c) 2013 Kaden & Partner AG
 */
package utils {
	 ...
	 */
	public class EnhancedLevelManager {
		
		static private var _instance:EnhancedLevelManager;
		...
		public function EnhancedLevelManager(ALevel:Class) {
		...
		static public function getInstance():EnhancedLevelManager {
			return _instance;
		}
